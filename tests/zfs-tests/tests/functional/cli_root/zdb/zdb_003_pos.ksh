...
for x in 0 1 ; do
	message_count=$($ZDB -l /dev/${DISK[$x]} | $GREP -c "same as label")
	(( $? != 0)) && log_fail "failed to get message_count from DISK[$x]"
	log_note "vdev $x: message_count $message_count"
	[ $message_count -ne ${message_count[$x]} ] && \
		log_fail "zdb incorrectly states which labels have the same config."

	config_count=$($ZDB -l /dev/${DISK[$x]} | $GREP -c features_for_read)
