...
# Verify "delalloc" is set on pending holes, then converted to unwritten
log_note "Verify 'delalloc' to 'unwritten' flags for pending holes"
fiemap_hole $BS 2
fiemap_verify -F "delalloc:all"
fiemap_verify -F "unwritten:=0"
fiemap_verify -s -F "delalloc:=0"
fiemap_verify -s -F "unwritten,merged:=1"
fiemap_remove

# Verify "unwritten" is set on holes
log_note "Verify 'unwritten' flag for holes"
fiemap_hole $BS 32
fiemap_verify -s -F "unwritten:all"
fiemap_remove
...
# Verify "merged" is set on holes merged in to extents.
...
# Verify "merged" is set on tail holes.
# Holes always merge so there must be only one.
...
u# Verify "merged" is set on head holes.
# Holes always merge so there must be only one.
