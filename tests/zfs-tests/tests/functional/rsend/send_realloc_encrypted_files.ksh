...
#
# NOTE: The following failures have been observed and need to be resolved:
# 1. Receiving the raw send in to a mounted dataset can hang in
#    zfsvfs_teardown() waiting for all iputs to complete.
# 2. The 'zfs mount $POOL/newfs' can fail with an invalid argument error
#    after the 'zpool load-key' load.
#
...
for i in {1..5}; do
	...
	log_must eval "zfs recv -F $POOL/newfs < $BACKDIR/fs@snap${this_snap}"

	# A briefly delay is sometimes needed between load-key and mount.
	log_must zfs load-key $POOL/newfs
	log_must zfs mount $POOL/newfs
	actual_cksum=$(recursive_cksum /$POOL/newfs)
	log_must zfs unmount $POOL/newfs
