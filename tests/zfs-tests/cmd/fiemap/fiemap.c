...
fiemap_verify_extents(fiemap_args_t *fa, verify_tree_type_t type)
{
	...
	for (int i = 0; i < fa->fa_fiemap->fm_mapped_extents; i++) {
		struct fiemap_extent *ext = &fa->fa_fiemap->fm_extents[i];

		if ((is_data && !(ext->fe_flags & FIEMAP_EXTENT_UNWRITTEN)) ||
		    (is_hole && (ext->fe_flags & FIEMAP_EXTENT_UNWRITTEN))) {
			space_reftree_add_seg(t, ext->fe_logical,
			    ext->fe_logical + ext->fe_length, -1);
		}
	}
...
fiemap_verify(fiemap_args_t *fa)
{
	...
	if (range_tree_space(rt) != statbuf.st_size) {
...
main(int argc, char *argv[])
{
	...
	while ((c = getopt(argc, argv, "achsvD:H:E:F:")) != -1) {
		switch (c) {
		case 'a':
