    ...
    nvlist_t *props, int flags)
{
	...
	if (!SPA_VERSION_IS_SUPPORTED(version)) {
		...
		uint64_t hostid = 0;
		char *hostname = NULL;
		char *import_hostname = NULL;

		hostname = fnvlist_lookup_string(config, ZPOOL_CONFIG_HOSTNAME);

		if (nvlist_exists(config, ZPOOL_CONFIG_IMPORT_HOSTNAME))
			import_hostname = fnvlist_lookup_string(config,
			    ZPOOL_CONFIG_IMPORT_HOSTNAME);

		if (import_hostname) {
			if (nvlist_exists(config, ZPOOL_CONFIG_IMPORT_HOSTID))
				hostid = fnvlist_lookup_uint64(config,
				    ZPOOL_CONFIG_IMPORT_HOSTID);

			(void) fprintf(stderr, gettext("Cannot import '%s': "
			    "pool is currently imported from %s (hostid: "
			    "0x%lx).\nExport the pool on the other system "
			    "before importing it with 'zpool import -f'.\n"),
			    name, import_hostname, (unsigned long)hostid);
		} else {
			uint64_t timestamp = fnvlist_lookup_uint64(config,
