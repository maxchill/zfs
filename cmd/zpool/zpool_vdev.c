...
draid_config_by_type(const char *type, uint64_t children)
{
	...
	case DRAIDCFG_OK:
		...
		error = vdev_draid_config_write_file(fullpath, key, cfg);
		if (error) {
			fprintf(stderr, gettext("Warning: Cannot write dRAID "
			    "configuration to %s: %d\n"), fullpath, error);
		}

		return (cfg);
	case DRAIDCFG_ERR_GROUPS_INVALID:
		fprintf(stderr,
		    gettext("Invalid number of dRAID groups %lu; must be "
		    "between 1 and %d\n"), groups, VDEV_DRAID_MAX_GROUPS);
		break;
	case DRAIDCFG_ERR_DATA_INVALID:
		fprintf(stderr,
		    gettext("Invalid dRAID group size %lu; must be between "
		    "0 and %d\n"), data + parity, VDEV_DRAID_MAX_GROUPSIZE);
		break;
	case DRAIDCFG_ERR_PARITY_INVALID:
		fprintf(stderr,
		    gettext("Invalid dRAID parity level %lu; must be between "
		    "1 and %d\n"), parity, VDEV_DRAID_MAX_PARITY);
		break;
	case DRAIDCFG_ERR_SPARES_INVALID:
		fprintf(stderr,
		    gettext("Invalid number of dRAID spares %lu; must be "
		    "between 1 and %d\n"), spares, VDEV_DRAID_MAX_SPARES);
		break;
	case DRAIDCFG_ERR_CHILDREN_INVALID:
		fprintf(stderr,
		    gettext("Invalid number of dRAID children %lu; must be "
