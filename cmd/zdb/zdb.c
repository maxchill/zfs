...
cksum_record_compare(const void *x1, const void *x2)
{
	...
	for (int i = 0; i < arraysize; i++) {
		error = AVL_CMP(l->cksum.zc_word[i], r->cksum.zc_word[i]);
		if (error)
			break;
	}
...
static void
dump_config_from_label(nvlist_t *config, cksum_record_t *rec,
    size_t buflen, int l)
{
	if (dump_opt['q'])
		return;

	if ((dump_opt['l'] < 3) && (first_label(rec) != l))
		return;
...
dump_label(const char *dev)
{
	...
	label_t labels[VDEV_LABELS] = { 0 };
	uint64_t psize, ashift;
	struct stat64 statbuf;
	boolean_t label_found = B_FALSE;
	avl_tree_t config_tree;
	avl_tree_t uberblock_tree;
	void *node, *cookie = NULL;
	int fd;
	...
	for (int l = 0; l < VDEV_LABELS; l++) {
		...
		for (int i = 0; i < VDEV_UBERBLOCK_COUNT(&vd); i++) {
			uint64_t uoff = VDEV_UBERBLOCK_OFFSET(&vd, i);
			uberblock_t *ub = (void *)((char *)&labels[l] + uoff);
	...
	for (int l = 0; l < VDEV_LABELS; l++) {
		...
		if (label->config_nv) {
			dump_config_from_label(label->config_nv,
			    label->config, buflen, l);
		} else {
			if (!dump_opt['q'])
		...
		if (dump_opt['u'])
			dump_label_uberblocks(&labels[l], ashift, l);
