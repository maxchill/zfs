...
dsl_scan_prefetch_thread(void *arg)
{
	...
	while (!scn->scn_prefetch_stop) {
		...
		while (!scn->scn_prefetch_stop &&
		    ...
		    spa->spa_scrub_inflight >= scn->scn_max_bytes_inflight)) {
...
dsl_scan_sync(dsl_pool_t *dp, dmu_tx_t *tx)
{
	...
	if (!scn->scn_clearing && scn->scn_done_txg == 0) {
		 ...
		 * scanning operations. Limits for the issuing phase are done
		 * per top-level vdev and are handled separately.
		 */
		scn->scn_max_bytes_inflight = nr_leaves * zfs_scan_vdev_limit;
    ...
    const zbookmark_phys_t *zb, dsl_scan_io_queue_t *queue)
{
	...
	if (queue == NULL) {
		mutex_enter(&spa->spa_scrub_lock);
		while (spa->spa_scrub_inflight >= scn->scn_max_bytes_inflight)
			cv_wait(&spa->spa_scrub_io_cv, &spa->spa_scrub_lock);
		spa->spa_scrub_inflight += BP_GET_PSIZE(bp);
		mutex_exit(&spa->spa_scrub_lock);
	} else {
		...
		mutex_enter(q_lock);
		while (queue->q_inflight_bytes >= zfs_scan_vdev_limit)
